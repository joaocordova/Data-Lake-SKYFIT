# -*- coding: utf-8 -*-
"""
Loader OTIMIZADO: EVO Members Bronze → STG

Usa COPY + Temp Table para performance máxima.

Uso:
    cd C:/skyfit-datalake/evo_members
    python src/loaders/load_evo_members_stg_fast.py --workers 4
"""
import argparse
import gzip
import io
...
log_file = LOG_DIR / f"load_members_fast_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
    level=logging.INFO,
...
class FastMembersLoader:
    ...
    def run(self, run_id: Optional[str] = None, all_runs: bool = False) -> Dict[str, Any]:
        logger.info("="*60)
        logger.info("EVO MEMBERS - LOADER OTIMIZADO (COPY)")
        logger.info("="*60)
            ...
            with ThreadPoolExecutor(max_workers=self.workers) as executor:
                ...
                for future in as_completed(futures):
                    try:
                        ...
                        total_records += count
                        processed += 1
                        if processed % 50 == 0 or processed == len(parts):
                            logger.info(f"  [{processed}/{len(parts)}] {total_records:,} membros carregados")
                    except Exception as e:
                        errors += 1
...
def main():
    parser = argparse.ArgumentParser(description="Loader otimizado para EVO Members")
    ...
    result = FastMembersLoader(workers=args.workers).run(run_id=args.run_id, all_runs=args.all_runs)
